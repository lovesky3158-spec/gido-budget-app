...
export default function PersonBadge({ user, compact = false }: Props) {
  ...
  const map = {
    ...
    짱구: { label: "짱구", className: "bg-yellow-100 text-amber-700", icon: "/icons/jjanggu.png", emoji: "🧒" },
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 font-semibold ${compact ? "text-[11px]" : "text-xs"} ${item.className}`}>
      {item.icon ? (
